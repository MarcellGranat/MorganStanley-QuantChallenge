...
weekly_pw_design_df <- weekly_design_df |> # p:previous year combined, w:weights added
  ...
  ungroup() |> 
  mutate(
    # Koyck weights + "undersampling"
    case_wts = .75 ^ (max(year) - year) * min(n) / n, # gamma = 0.75
...
corn_prediction_no_wts_training <- weekly_pw_selected_design_df |> 
  select(- case_wts) |> 
  filter(year >= 2006)

.write(corn_prediction_no_wts_training)
...
weekly_training_rset_75 <- weekly_design_folds |> 
  filter(year < 2010, row_number() > 8) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

weekly_testing_rset_75 <- weekly_design_folds |> 
  filter(year >= 2010) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

.write(weekly_training_rset_75, weekly_testing_rset_75)

# without case_wts

weekly_design_folds_no_wts <- weekly_pw_selected_design_df |> 
  select(- case_wts) |> 
  group_by(year) |> 
  nest() |> 
  arrange(year) |>
...
weekly_training_no_wts_rset <- weekly_design_folds_no_wts |> 
  filter(year < 2010, row_number() > 8) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

weekly_testing_no_wts_rset <- weekly_design_folds_no_wts |> 
  filter(year >= 2010) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

.write(weekly_training_no_wts_rset, weekly_testing_no_wts_rset)

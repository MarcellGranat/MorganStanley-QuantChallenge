...
weekly_pw_design_df <- weekly_design_df |> 
  ...
  ungroup() |> 
  mutate(
    case_wts = .95 ^ (max(year) - year) * min(n) / n, # Koyck weights + "undersampling"
...
weekly_training_rset_95 <- weekly_design_folds |> 
  filter(year < 2010, row_number() > 8) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

weekly_testing_rset_95 <- weekly_design_folds |> 
  filter(year >= 2010) |> 
  mutate(splits = map2(analysis, assessment, make_splits)) %$%
  manual_rset(splits, as.character(year))

.write(weekly_training_rset_95, weekly_testing_rset_95)

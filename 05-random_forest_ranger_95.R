.read(weekly_training_rset_95)

rand_forest_ranger_spec <- rand_forest(mtry = tune(), min_n = tune()) %>%
  set_engine('ranger') %>%
  set_mode('regression')

weekly_recipe <- recipe(yield ~ ., data = analysis(weekly_training_rset_95$splits[[1]])) |> 
  step_rm(county) |> 
...
gri <- tune_grid(
  wf, 
  resamples = weekly_training_rset_95,
  metrics = metric_set(rsq, rmse, msd, mape),
  grid = 90
)

rand_forest_ranger_95_wf <- wf |> 
  finalize_workflow(
    parameters = select_best(gri, "rmse")
  )

.write(rand_forest_ranger_95_wf)
